...
LOGO_PATH = Path("logo_santacruz.png")
...
def achar_cabecalho_pedido(arquivo):
    bruto = pd.read_excel(arquivo, header=None, engine="openpyxl")
...
def carregar_pedido(arquivo):
    ...
    df = pd.read_excel(arquivo, header=header, engine="openpyxl")
...
st.markdown("""
<div class="footer">
    Gerador de Pedidos • V10 • Plataforma para processamento e geração de pedidos
</div>
""", unsafe_allow_html=True)